...
#      - create_link_check_table.sql (link checker results table)
#      - create_code_archive_table.sql (icecave archival status table)
#      - add_short_name_column.sql (short_name from GitHub repo names)
#      - create_abuse_blocker_tables.sql (per-IP rate limiter tables)
...
if [[ -f "$CODE_ARCHIVE_SQL" ]]; then
    ...
    print_success "code_archive table created"
else
    print_warning "Migration script not found: $CODE_ARCHIVE_SQL"
fi
...
if [[ -f "$ABUSE_BLOCKER_SQL" ]]; then
    ...
    print_success "abuse blocker tables created (ip_error_event, ip_block)"
else
    print_warning "Migration script not found: $ABUSE_BLOCKER_SQL"
fi
